...
# =============================================================================
# GeoBasisMLPKrigingNet
# -----------------------------------------------------------------------------
# Point-based model with:
# - strong residual MLP tabular branch
# - learned spatial radial-basis layer with train-initialized centers
# - light anisotropic residual kriging-like correction
#
# Plain-language flow:
# 1. Learn a robust tabular base prediction.
# 2. Add a conservative spatial basis correction from learned coordinate anchors.
# 3. Predict target and neighbor bases.
...
sample_basis_centers <- function(coords_train, n_centers = 32, seed = 123) {
  set.seed(seed)
...
prepare_geobasismlp_fold <- function(context,
                                     calibration_df,
                                     train_idx,
                                     val_idx,
                                     test_idx,
  ...
  init_centers <- sample_basis_centers(coords_train_s, n_centers = n_centers, seed = center_seed)
...
GeoBasisMLPKrigingNet <- nn_module(
  "GeoBasisMLPKrigingNet",
  initialize = function(c_tab,
    ...
    self$target_head <- GeoBasisScalarHead(d = d, hidden = 96, dropout = dropout)
    self$neighbor_head <- GeoBasisScalarHead(d = d, hidden = 96, dropout = dropout)
...
predict_geobasismlp <- function(model,
                                X_query,
                                coords_query,
                                neighbor_idx,
                                X_ref,
  ...
  with_no_grad({
    for (s in seq(1, n, by = batch_size)) {
      ...
      if (length(pred) != length(idx)) {
        stop(sprintf(
          "GeoBasisMLPKrigingNet prediction length mismatch: expected %d values, got %d.",
...
train_geobasismlp_one_fold <- function(fd,
                                       epochs = 40,
                                       lr = 2e-4,
                                       wd = 1e-3,
                                       batch_size = 96,
  ...
  if (warmup_epochs > 0) {
    ...
    for (ep in seq_len(warmup_epochs)) {
      ...
      cat(sprintf("[GeoBasisMLP Warmup] Epoch %d/%d | train_loss=%.4f\n",
  ...
  for (ep in seq_len(epochs)) {
    ...
    for (batch_id in seq_along(batches)) {
      ...
      if (batch_id %% 10 == 0 || batch_id == length(batches)) {
        cat(sprintf("[GeoBasisMLP] Epoch %d | batch %d/%d | batch_loss=%.4f\n",
                    ep, batch_id, length(batches), loss$item()))
      }
    }

    val_pred_s <- predict_geobasismlp(
      model = model,
      X_query = Xva,
      coords_query = Cva,
    ...
    cat(sprintf("[GeoBasisMLP] Epoch %d complete | train_loss=%.4f | val_mse=%.4f\n",
  ...
  preds_scaled <- predict_geobasismlp(
    model = model,
    X_query = Xte,
    coords_query = Cte,
...
geobasismlp_quick_params <- modifyList(
  geobasismlp_params,
  list(
    epochs = 20,
    batch_size = 64,
...
run_geobasismlp_on_fixed_benchmark <- function(benchmark,
                                               context = wadoux_context,
                                               model_params = geobasismlp_params,
                                               K_neighbors = 12,
                                               n_centers = 32) {
  ...
  for (i in seq_along(benchmark$splits)) {
    ...
    cat(sprintf("\n[GeoBasisMLP Fair] split %s | K_neighbors=%d | centers=%d\n",
    ...
    out <- do.call(train_geobasismlp_one_fold, c(list(fd = fd), model_params))
...
run_geobasismlp_vs_cubist_fair <- function(context = wadoux_context,
                                           sample_size = 250,
                                           ...
                                           max_splits = 5,
                                           seed = 123,
                                           model_params = geobasismlp_quick_params,
                                           K_neighbors = 12,
                                           n_centers = 32,
                                           cubist_committees = 50,
                                           cubist_neighbors = 5,
                                           results_dir = "results/geobasismlp_vs_cubist",
                                           save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoBasisMLPKrigingNet vs Cubist BENCHMARK\n")
  cat("========================================\n")
  ...
  geo_res <- run_geobasismlp_on_fixed_benchmark(
    benchmark = benchmark,
    context = context,
  ...
  if (save_outputs) {
    ...
    write.csv(geo_res, file.path(results_dir, "geobasismlp_results.csv"), row.names = FALSE)
    write.csv(final, file.path(results_dir, "geobasismlp_vs_cubist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geobasismlp_vs_cubist_summary.csv"), row.names = FALSE)
  }

  final
}

run_geobasismlp_vs_cubist_confirmation <- function(context = wadoux_context,
                                                   sample_size = 300,
                                                   sampling = "simple_random",
                                                   n_folds = 10,
                                                   val_dist_km = 350,
                                                   ...
                                                   cubist_committees = 50,
                                                   cubist_neighbors = 5,
                                                   results_dir = "results/geobasismlp_vs_cubist_confirmation",
                                                   save_outputs = TRUE) {
  run_geobasismlp_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
