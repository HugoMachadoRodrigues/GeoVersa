...
# - smooth spatial basis branch
# - differentiable soft decision tree over fused tabular+spatial context
# - light anisotropic residual kriging-like correction
#
# Plain-language flow:
# 1. Learn a robust global representation from covariates and coordinates.
# 2. Route each point through a soft tree that mimics local rule partitions.
# 3. Add the tree-induced local representation conservatively to the base.
...
sample_softtree_centers <- function(coords_train, n_centers = 32, seed = 123) {
  set.seed(seed)
...
prepare_geosofttree_fold <- function(context,
                                     calibration_df,
                                     train_idx,
                                     val_idx,
                                     test_idx,
  ...
  init_centers <- sample_softtree_centers(coords_train_s, n_centers = n_centers, seed = center_seed)
...
GeoSoftTreeRouter <- nn_module(
  ...
  initialize = function(d = 160,
    ...
    self$n_internal <- as.integer(2^depth - 1)
    self$n_leaves <- as.integer(2^depth)
    self$tree_scale <- tree_scale
    self$temperature <- temperature
    self$route_net <- nn_sequential(
      nn_linear(2 * d, route_hidden),
      nn_gelu(),
      nn_dropout(dropout),
      nn_linear(route_hidden, self$n_internal)
    )
    self$leaf_bank <- nn_parameter(torch_randn(c(self$n_leaves, d)) * 0.05)
  },
  compute_leaf_probs = function(gates) {
    nodes <- list(list(prob = torch_ones_like(gates[, 1]), idx = 1L))

    for (level in seq_len(self$depth)) {
      new_nodes <- list()
      for (node in nodes) {
        gate <- gates[, node$idx]
        new_nodes[[length(new_nodes) + 1L]] <- list(
          prob = node$prob * (1 - gate),
          idx = 2L * node$idx
        )
        new_nodes[[length(new_nodes) + 1L]] <- list(
          prob = node$prob * gate,
    ...
    torch_stack(lapply(nodes, function(node) node$prob), dim = 2)
  },
  forward = function(z_tab, z_basis) {
    gate_logits <- self$route_net(torch_cat(list(z_tab, z_basis), dim = 2))
    gates <- torch_sigmoid(gate_logits / self$temperature)
    leaf_probs <- self$compute_leaf_probs(gates)
    z_tree <- torch_matmul(leaf_probs, self$leaf_bank)
    list(
      z_tree = self$tree_scale * z_tree,
      leaf_probs = leaf_probs
    )
  }
)

GeoSoftTreeScalarHead <- nn_module(
  "GeoSoftTreeScalarHead",
...
GeoSoftTreeKrigingNet <- nn_module(
  "GeoSoftTreeKrigingNet",
  initialize = function(c_tab,
    ...
    self$tree_router <- GeoSoftTreeRouter(
      d = d,
      depth = tree_depth,
    ...
    self$target_head <- GeoSoftTreeScalarHead(d = d, hidden = 96, dropout = dropout)
    self$neighbor_head <- GeoSoftTreeScalarHead(d = d, hidden = 96, dropout = dropout)
  ...
  encode_points = function(x, coords) {
    ...
    tree <- self$tree_router(z_tab, z_basis)

    z <- z_tab +
      self$spatial_scale * z_basis +
      tree$z_tree +
      self$fuse_scale * self$fuse_residual(torch_cat(list(z_tab, z_basis, tree$z_tree), dim = 2))

    list(
      z = z,
      leaf_probs = tree$leaf_probs
    )
  },
  ...
  forward = function(x_target, coords_target, x_neighbors, coords_neighbors, y_neighbors, use_residual = TRUE) {
    ...
    list(
      ...
      delta = delta,
      beta = beta,
      leaf_probs = enc_target$leaf_probs
    )
  }
)

predict_geosofttree <- function(model,
                                X_query,
                                coords_query,
                                neighbor_idx,
                                X_ref,
  ...
  with_no_grad({
    for (s in seq(1, n, by = batch_size)) {
      ...
      if (length(pred) != length(idx)) {
        stop(sprintf(
          "GeoSoftTreeKrigingNet prediction length mismatch: expected %d values, got %d.",
...
train_geosofttree_one_fold <- function(fd,
                                       epochs = 40,
                                       lr = 2e-4,
                                       wd = 1e-3,
                                       batch_size = 96,
  ...
  if (warmup_epochs > 0) {
    ...
    for (ep in seq_len(warmup_epochs)) {
      ...
      cat(sprintf("[GeoSoftTree Warmup] Epoch %d/%d | train_loss=%.4f\n",
  ...
  for (ep in seq_len(epochs)) {
    ...
    for (batch_id in seq_along(batches)) {
      ...
      if (batch_id %% 10 == 0 || batch_id == length(batches)) {
        cat(sprintf("[GeoSoftTree] Epoch %d | batch %d/%d | batch_loss=%.4f\n",
                    ep, batch_id, length(batches), loss$item()))
      }
    }

    val_pred_s <- predict_geosofttree(
      model = model,
      X_query = Xva,
      coords_query = Cva,
    ...
    cat(sprintf("[GeoSoftTree] Epoch %d complete | train_loss=%.4f | val_mse=%.4f\n",
  ...
  preds_scaled <- predict_geosofttree(
    model = model,
    X_query = Xte,
    coords_query = Cte,
...
geosofttreekrigingnet_params <- list(
  epochs = 40,
  lr = 2e-4,
  wd = 1e-3,
  batch_size = 96,
...
geosofttreekrigingnet_quick_params <- modifyList(
  geosofttreekrigingnet_params,
  list(
    epochs = 20,
    batch_size = 64,
    patience = 5,
...
geosofttreekrigingnet_conservative_quick_params <- modifyList(
  geosofttreekrigingnet_quick_params,
  list(
    lr = 1e-4,
    spatial_scale = 0.25,
    tree_scale = 0.15,
    fuse_scale = 0.05,
    tree_temperature = 1.20,
    beta_init = -5,
    warmup_epochs = 6,
...
run_geosofttree_on_fixed_benchmark <- function(benchmark,
                                               context = wadoux_context,
                                               model_params = geosofttreekrigingnet_params,
                                               K_neighbors = 12,
                                               n_centers = 32) {
  ...
  for (i in seq_along(benchmark$splits)) {
    ...
    cat(sprintf("\n[GeoSoftTree Fair] split %s | K_neighbors=%d | centers=%d\n",
    ...
    out <- do.call(train_geosofttree_one_fold, c(list(fd = fd), model_params))
...
run_geosofttreekrigingnet_vs_cubist_fair <- function(context = wadoux_context,
                                                     sample_size = 250,
                                                     ...
                                                     max_splits = 5,
                                                     seed = 123,
                                                     model_params = geosofttreekrigingnet_quick_params,
                                                     K_neighbors = 12,
                                                     n_centers = 32,
                                                     cubist_committees = 50,
                                                     cubist_neighbors = 5,
                                                     results_dir = "results/geosofttreekrigingnet_vs_cubist",
                                                     save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoSoftTreeKrigingNet vs Cubist BENCHMARK\n")
  cat("========================================\n")
  ...
  geo_res <- run_geosofttree_on_fixed_benchmark(
    benchmark = benchmark,
    context = context,
  ...
  if (save_outputs) {
    ...
    write.csv(geo_res, file.path(results_dir, "geosofttreekrigingnet_results.csv"), row.names = FALSE)
    write.csv(final, file.path(results_dir, "geosofttreekrigingnet_vs_cubist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geosofttreekrigingnet_vs_cubist_summary.csv"), row.names = FALSE)
  }

  final
}

run_geosofttreekrigingnet_vs_cubist_confirmation <- function(context = wadoux_context,
                                                             sample_size = 300,
                                                             sampling = "simple_random",
                                                             n_folds = 10,
                                                             val_dist_km = 350,
                                                             ...
                                                             cubist_committees = 50,
                                                             cubist_neighbors = 5,
                                                             results_dir = "results/geosofttreekrigingnet_vs_cubist_confirmation",
                                                             save_outputs = TRUE) {
  run_geosofttreekrigingnet_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
...
make_geosofttree_v2_variants <- function() {
  list(
    GeoSoftTree_Base = list(
      params = geosofttreekrigingnet_quick_params,
      K_neighbors = 12,
      n_centers = 32
    ),
    GeoSoftTree_Conservative = list(
      params = geosofttreekrigingnet_conservative_quick_params,
      K_neighbors = 12,
      n_centers = 32
    )
  )
...
run_geosofttree_v2_search <- function(context = wadoux_context,
                                      sample_size = 250,
                                      sampling = "simple_random",
                                      n_folds = 5,
                                      val_dist_km = 350,
                                      ...
                                      cubist_committees = 50,
                                      cubist_neighbors = 5,
                                      results_dir = "results/geosofttreekrigingnet_v2_search",
                                      save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoSoftTree V2 SEARCH\n")
  cat("========================================\n")
  ...
  variants <- make_geosofttree_v2_variants()
  all_results <- list()
  ...
  if (save_outputs) {
    write.csv(final, file.path(results_dir, "geosofttree_v2_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geosofttree_v2_summary.csv"), row.names = FALSE)
  }
