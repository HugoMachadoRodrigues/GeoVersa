...
# GeoFlowKrigingNet
# -----------------------------------------------------------------------------
# Point-based geostatistical conditional normalizing flow with:
# - strong tabular + spatial basis context encoder
# - monotone scalar conditional flow for the target distribution
# - light anisotropic residual kriging-like correction for point prediction
#
# Plain-language flow:
# 1. Encode covariates and coordinates into a context vector.
# 2. Learn an invertible conditional transformation from y to latent z ~ N(0,1).
# 3. Use the conditional median (z = 0) as a base prediction.
# 4. Compute neighbor residuals on that base prediction.
# 5. Apply a light anisotropic spatial correction.
# =============================================================================
...
sample_flow_centers <- function(coords_train, n_centers = 32, seed = 123) {
  set.seed(seed)
...
prepare_geoflow_fold <- function(context,
                                 calibration_df,
                                 train_idx,
                                 val_idx,
                                 test_idx,
  ...
  init_centers <- sample_flow_centers(coords_train_s, n_centers = n_centers, seed = center_seed)
...
GeoFlowContextEncoder <- nn_module(
  "GeoFlowContextEncoder",
...
ConditionalMonotoneFlow <- nn_module(
  "ConditionalMonotoneFlow",
  initialize = function(d_context = 160, n_steps = 4, hidden = 128, step_clip = 1.0) {
    self$n_steps <- n_steps
    self$step_clip <- step_clip
    self$param_head <- nn_sequential(
      nn_linear(d_context, hidden),
      nn_gelu(),
      nn_linear(hidden, 3 * n_steps)
    )
    self$init_head <- nn_sequential(
      nn_linear(d_context, hidden),
      nn_gelu(),
      nn_linear(hidden, 1)
    )
  ...
  flow_forward = function(y, h) {
    params <- self$param_head(h)
    params <- reshape_safe(params, c(y$shape[1], self$n_steps, 3))
    z <- y
    logdet <- torch_zeros_like(y)
    ...
    for (i in seq_len(self$n_steps)) {
      ...
      b <- nnf_softplus(b_raw) + 1e-3
      ab <- 0.9 * torch_tanh(a_raw)
      t <- b * z + c_raw
      tanh_t <- torch_tanh(t)
      sech2_t <- 1 - tanh_t^2

      z <- z + (ab / b) * tanh_t
      logdet <- logdet + torch_log(1 + ab * sech2_t + 1e-6)
    }

    list(z = z, logdet = logdet)
  },

  inverse_to_median = function(h, n_iter = 20) {
    y <- flatten_safe(self$init_head(h))
    target_z <- torch_zeros_like(y)

    for (i in seq_len(n_iter)) {
      fwd <- self$flow_forward(y, h)
      resid <- fwd$z - target_z
      deriv <- torch_exp(fwd$logdet)
      step <- torch_clamp(resid / (deriv + 1e-6), min = -self$step_clip, max = self$step_clip)
      y <- y - step
    }
...
GeoFlowKrigingNet <- nn_module(
  ...
  initialize = function(c_tab,
                        ...
                        beta_init = -4) {
    self$d <- d
    self$context_encoder <- GeoFlowContextEncoder(
      c_tab = c_tab,
      init_centers = init_centers,
      d = d,
    ...
    self$flow <- ConditionalMonotoneFlow(
      d_context = d,
      n_steps = flow_steps,
      hidden = flow_hidden,
      step_clip = 1.0
  ...
  predict_base_from_context = function(h) {
    self$flow$inverse_to_median(h, n_iter = 20)
  },

  forward = function(x_target, coords_target, x_neighbors, coords_neighbors, y_neighbors, y_target = NULL, use_residual = TRUE) {
    h_target <- self$context_encoder(x_target, coords_target)
    h_neighbors <- self$encode_neighbors(x_neighbors, coords_neighbors)

    base_target <- self$predict_base_from_context(h_target)
    ...
    base_neighbors <- self$predict_base_from_context(h_neighbors_flat)
    base_neighbors <- reshape_safe(base_neighbors, c(dims_neighbors[1], dims_neighbors[2]))
    ...
    flow_target <- NULL
    if (!is.null(y_target)) {
      flow_target <- self$flow$flow_forward(y_target, h_target)
    }
...
gaussian_nll_from_flow <- function(flow_out) {
  z <- flow_out$z
  logdet <- flow_out$logdet
  mean(0.5 * z^2 + 0.5 * log(2 * pi) - logdet)
}

predict_geoflow <- function(model,
                            X_query,
                            coords_query,
                            neighbor_idx,
                            X_ref,
  ...
  with_no_grad({
    for (s in seq(1, n, by = batch_size)) {
      ...
      pred <- as.numeric(model(xb, cb, xnb, cnb, ynb, use_residual = TRUE)$pred$to(device = "cpu"))
      if (length(pred) != length(idx)) {
        stop(sprintf(
          "GeoFlowKrigingNet prediction length mismatch: expected %d values, got %d.",
...
train_geoflow_one_fold <- function(fd,
                                   epochs = 40,
                                   lr = 2e-4,
                                   wd = 1e-3,
                                   batch_size = 96,
  ...
  if (warmup_epochs > 0) {
    ...
    for (ep in seq_len(warmup_epochs)) {
      ...
      for (batch_id in seq_along(batches)) {
        ...
        out <- model(xb, cb, xnb, cnb, ynb, y_target = yb, use_residual = FALSE)
        loss <- gaussian_nll_from_flow(out$flow_target) + base_loss_weight * huber_loss(yb, out$base_pred)
      ...
      cat(sprintf("[GeoFlow Warmup] Epoch %d/%d | train_loss=%.4f\n",
  ...
  for (ep in seq_len(epochs)) {
    ...
    for (batch_id in seq_along(batches)) {
      ...
      out <- model(xb, cb, xnb, cnb, ynb, y_target = yb, use_residual = TRUE)
      nll <- gaussian_nll_from_flow(out$flow_target)
      loss <- nll_weight * nll + huber_loss(yb, out$pred) + base_loss_weight * huber_loss(yb, out$base_pred)
      ...
      if (batch_id %% 10 == 0 || batch_id == length(batches)) {
        cat(sprintf("[GeoFlow] Epoch %d | batch %d/%d | batch_loss=%.4f\n",
                    ep, batch_id, length(batches), loss$item()))
      }
    }

    val_pred_s <- predict_geoflow(
      model = model,
      X_query = Xva,
      coords_query = Cva,
    ...
    cat(sprintf("[GeoFlow] Epoch %d complete | train_loss=%.4f | val_mse=%.4f\n",
...
geoflowkrigingnet_quick_params <- modifyList(
  geoflowkrigingnet_params,
  list(
    epochs = 20,
    batch_size = 64,
    patience = 5,
...
run_geoflow_on_fixed_benchmark <- function(benchmark,
                                           context = wadoux_context,
                                           model_params = geoflowkrigingnet_params,
                                           K_neighbors = 12,
                                           n_centers = 32) {
  ...
  for (i in seq_along(benchmark$splits)) {
    ...
    cat(sprintf("\n[GeoFlow Fair] split %s | K_neighbors=%d | centers=%d\n",
    ...
    out <- do.call(train_geoflow_one_fold, c(list(fd = fd), model_params))
...
run_geoflowkrigingnet_vs_cubist_fair <- function(context = wadoux_context,
                                                 sample_size = 250,
                                                 ...
                                                 max_splits = 5,
                                                 seed = 123,
                                                 model_params = geoflowkrigingnet_quick_params,
                                                 K_neighbors = 12,
                                                 n_centers = 32,
                                                 cubist_committees = 50,
                                                 cubist_neighbors = 5,
                                                 results_dir = "results/geoflowkrigingnet_vs_cubist",
                                                 save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoFlowKrigingNet vs Cubist BENCHMARK\n")
  cat("========================================\n")
  ...
  flow_res <- run_geoflow_on_fixed_benchmark(
    benchmark = benchmark,
    context = context,
  ...
  final <- bind_rows(cubist_res, flow_res)
  summary_tbl <- final %>%
    group_by(model) %>%
    summarise(
  ...
  if (save_outputs) {
    ...
    write.csv(flow_res, file.path(results_dir, "geoflowkrigingnet_results.csv"), row.names = FALSE)
    write.csv(final, file.path(results_dir, "geoflowkrigingnet_vs_cubist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geoflowkrigingnet_vs_cubist_summary.csv"), row.names = FALSE)
  }

  final
}

run_geoflowkrigingnet_vs_cubist_confirmation <- function(context = wadoux_context,
                                                         sample_size = 300,
                                                         sampling = "simple_random",
                                                         n_folds = 10,
                                                         val_dist_km = 350,
                                                         ...
                                                         cubist_committees = 50,
                                                         cubist_neighbors = 5,
                                                         results_dir = "results/geoflowkrigingnet_vs_cubist_confirmation",
                                                         save_outputs = TRUE) {
  run_geoflowkrigingnet_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
