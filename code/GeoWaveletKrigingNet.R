...
# - multiresolution wavelet basis encoder over coordinates
# - conservative fusion between tabular and wavelet branches
# - light anisotropic residual kriging-like correction
#
# Plain-language flow:
# 1. Learn a stable global representation from covariates.
# 2. Learn localized spatial effects with multiscale wavelet bases.
# 3. Fuse both branches conservatively into a base prediction.
...
sample_wavelet_centers <- function(coords_train, n_centers = 32, seed = 123) {
  set.seed(seed)
...
prepare_geowavelet_fold <- function(context,
                                    calibration_df,
                                    train_idx,
                                    val_idx,
                                    test_idx,
  ...
  init_centers <- sample_wavelet_centers(coords_train_s, n_centers = n_centers, seed = center_seed)
...
GeoWaveletSpatialEncoder <- nn_module(
  "GeoWaveletSpatialEncoder",
  initialize = function(init_centers,
    ...
    self$n_scales <- length(scales)
    self$centers <- nn_parameter(centers_tensor$clone())
    self$log_scales <- nn_parameter(torch_log(torch_tensor(as.numeric(scales), dtype = torch_float())))
    self$head <- nn_sequential(
      nn_linear(self$n_centers * self$n_scales + 2, hidden),
      nn_gelu(),
      nn_dropout(dropout),
  ...
  forward = function(coords) {
    ...
    dist2 <- dx^2 + dy^2
    scales <- nnf_softplus(self$log_scales) + 1e-6

    feats <- vector("list", self$n_scales + 1)
    for (i in seq_len(self$n_scales)) {
      s <- scales[i]
      ratio <- dist2 / (s^2 + 1e-8)
      wave_i <- (1 - ratio) * torch_exp(-0.5 * ratio)
      feats[[i]] <- wave_i
    }
    feats[[self$n_scales + 1]] <- coords
    feat <- torch_cat(feats, dim = 2)
    self$head(feat)
  }
)

GeoWaveletScalarHead <- nn_module(
  "GeoWaveletScalarHead",
...
GeoWaveletKrigingNet <- nn_module(
  "GeoWaveletKrigingNet",
  initialize = function(c_tab,
                        init_centers,
                        d = 160,
                        tab_hidden = c(192, 96),
                        wavelet_scales = c(0.5, 1.0, 2.0),
                        wavelet_hidden = 128,
                        fusion_hidden = 160,
                        spatial_scale = 0.35,
                        fuse_scale = 0.20,
    ...
    self$wavelet_encoder <- GeoWaveletSpatialEncoder(
      init_centers = init_centers,
      out_dim = d,
      scales = wavelet_scales,
      hidden = wavelet_hidden,
      dropout = dropout
    )
    ...
    self$target_head <- GeoWaveletScalarHead(d = d, hidden = 96, dropout = dropout)
    self$neighbor_head <- GeoWaveletScalarHead(d = d, hidden = 96, dropout = dropout)
  ...
  encode_points = function(x, coords) {
    ...
    z_wave <- self$wavelet_encoder(coords)
    z_tab +
      self$spatial_scale * z_wave +
      self$fuse_scale * self$fuse_residual(torch_cat(list(z_tab, z_wave), dim = 2))
  },
...
predict_geowavelet <- function(model,
                               X_query,
                               coords_query,
                               neighbor_idx,
                               X_ref,
  ...
  with_no_grad({
    for (s in seq(1, n, by = batch_size)) {
      ...
      if (length(pred) != length(idx)) {
        stop(sprintf(
          "GeoWaveletKrigingNet prediction length mismatch: expected %d values, got %d.",
...
train_geowavelet_one_fold <- function(fd,
                                      epochs = 40,
                                      lr = 2e-4,
                                      wd = 1e-3,
                                      batch_size = 96,
  ...
  model <- GeoWaveletKrigingNet(
    c_tab = ncol(Xtr),
    init_centers = fd$init_centers,
    d = d,
    tab_hidden = tab_hidden,
    wavelet_scales = wavelet_scales,
    wavelet_hidden = wavelet_hidden,
  ...
  if (warmup_epochs > 0) {
    warmup_params <- c(
      ...
      model$wavelet_encoder$parameters,
    ...
    for (ep in seq_len(warmup_epochs)) {
      ...
      cat(sprintf("[GeoWavelet Warmup] Epoch %d/%d | train_loss=%.4f\n",
  ...
  for (ep in seq_len(epochs)) {
    ...
    for (batch_id in seq_along(batches)) {
      ...
      if (batch_id %% 10 == 0 || batch_id == length(batches)) {
        cat(sprintf("[GeoWavelet] Epoch %d | batch %d/%d | batch_loss=%.4f\n",
                    ep, batch_id, length(batches), loss$item()))
      }
    }

    val_pred_s <- predict_geowavelet(
      model = model,
      X_query = Xva,
      coords_query = Cva,
    ...
    cat(sprintf("[GeoWavelet] Epoch %d complete | train_loss=%.4f | val_mse=%.4f\n",
  ...
  preds_scaled <- predict_geowavelet(
    model = model,
    X_query = Xte,
    coords_query = Cte,
...
geowaveletkrigingnet_params <- list(
  epochs = 40,
  lr = 2e-4,
  wd = 1e-3,
  batch_size = 96,
...
geowaveletkrigingnet_quick_params <- modifyList(
  geowaveletkrigingnet_params,
  list(
    epochs = 20,
    batch_size = 64,
    patience = 5,
...
geowaveletkrigingnet_conservative_params <- modifyList(
  geowaveletkrigingnet_params,
  list(
    lr = 1e-4,
    spatial_scale = 0.20,
...
geowaveletkrigingnet_conservative_quick_params <- modifyList(
  geowaveletkrigingnet_quick_params,
  list(
    lr = 1e-4,
    spatial_scale = 0.20,
    fuse_scale = 0.10,
...
run_geowavelet_on_fixed_benchmark <- function(benchmark,
                                              context = wadoux_context,
                                              model_params = geowaveletkrigingnet_params,
                                              K_neighbors = 12,
                                              n_centers = 32) {
  ...
  for (i in seq_along(benchmark$splits)) {
    ...
    cat(sprintf("\n[GeoWavelet Fair] split %s | K_neighbors=%d | centers=%d\n",
    ...
    out <- do.call(train_geowavelet_one_fold, c(list(fd = fd), model_params))
...
run_geowaveletkrigingnet_vs_cubist_fair <- function(context = wadoux_context,
                                                    sample_size = 250,
                                                    ...
                                                    max_splits = 5,
                                                    seed = 123,
                                                    model_params = geowaveletkrigingnet_quick_params,
                                                    K_neighbors = 12,
                                                    n_centers = 32,
                                                    cubist_committees = 50,
                                                    cubist_neighbors = 5,
                                                    results_dir = "results/geowaveletkrigingnet_vs_cubist",
                                                    save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoWaveletKrigingNet vs Cubist BENCHMARK\n")
  cat("========================================\n")
  ...
  geo_res <- run_geowavelet_on_fixed_benchmark(
    benchmark = benchmark,
    context = context,
  ...
  if (save_outputs) {
    ...
    write.csv(geo_res, file.path(results_dir, "geowaveletkrigingnet_results.csv"), row.names = FALSE)
    write.csv(final, file.path(results_dir, "geowaveletkrigingnet_vs_cubist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geowaveletkrigingnet_vs_cubist_summary.csv"), row.names = FALSE)
  }

  final
}

run_geowaveletkrigingnet_vs_cubist_confirmation <- function(context = wadoux_context,
                                                            sample_size = 300,
                                                            sampling = "simple_random",
                                                            n_folds = 10,
                                                            val_dist_km = 350,
                                                            ...
                                                            cubist_committees = 50,
                                                            cubist_neighbors = 5,
                                                            results_dir = "results/geowaveletkrigingnet_vs_cubist_confirmation",
                                                            save_outputs = TRUE) {
  run_geowaveletkrigingnet_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
...
run_geowaveletkrigingnet_conservative_confirmation <- function(context = wadoux_context,
                                                               sample_size = 300,
                                                               sampling = "simple_random",
                                                               n_folds = 10,
                                                               val_dist_km = 350,
                                                               ...
                                                               cubist_committees = 50,
                                                               cubist_neighbors = 5,
                                                               results_dir = "results/geowaveletkrigingnet_conservative_confirmation",
                                                               save_outputs = TRUE) {
  run_geowaveletkrigingnet_vs_cubist_confirmation(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
    ...
    max_splits = max_splits,
    seed = seed,
    model_params = geowaveletkrigingnet_conservative_params,
    K_neighbors = K_neighbors,
    n_centers = n_centers,
    cubist_committees = cubist_committees,
    cubist_neighbors = cubist_neighbors,
...
make_geowavelet_v2_variants <- function() {
  list(
    GeoWavelet_Base = list(
      params = geowaveletkrigingnet_quick_params,
      K_neighbors = 12,
      n_centers = 32
    ),
    GeoWavelet_Conservative = list(
      params = geowaveletkrigingnet_conservative_quick_params,
      K_neighbors = 12,
      n_centers = 32
    )
  )
...
run_geowavelet_v2_search <- function(context = wadoux_context,
                                     sample_size = 250,
                                     sampling = "simple_random",
                                     n_folds = 5,
                                     val_dist_km = 350,
                                     ...
                                     cubist_committees = 50,
                                     cubist_neighbors = 5,
                                     results_dir = "results/geowaveletkrigingnet_v2_search",
                                     save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoWavelet V2 SEARCH\n")
  cat("========================================\n")
  ...
  variants <- make_geowavelet_v2_variants()
  all_results <- list()
  ...
  if (save_outputs) {
    write.csv(final, file.path(results_dir, "geowavelet_v2_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geowavelet_v2_summary.csv"), row.names = FALSE)
  }
