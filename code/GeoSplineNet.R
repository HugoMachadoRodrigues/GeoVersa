...
# - tabular predictor branch
# - smooth spatial branch built from radial spline-like basis functions
# - anisotropic residual kriging-like correction
#
# Plain-language flow:
# 1. Encode covariates at the target point.
# 2. Encode target coordinates through a smooth radial basis expansion.
# 3. Fuse both into a base prediction.
# 4. Predict neighbor bases.
...
sample_rbf_centers <- function(coords_train, n_centers = 32, seed = 123) {
  set.seed(seed)
...
prepare_geospline_fold <- function(context,
                                   calibration_df,
                                   train_idx,
                                   val_idx,
                                   test_idx,
  ...
  centers <- sample_rbf_centers(coords_train_s, n_centers = n_centers, seed = center_seed)
...
expand_centers_array <- function(centers, B) {
  M <- nrow(centers)
  out <- array(0, dim = c(B, M, 2))
  for (i in seq_len(B)) {
    out[i, , ] <- centers
  }
  out
...
RadialSplineEncoder <- nn_module(
  "RadialSplineEncoder",
  initialize = function(n_centers, out_dim = 160, dropout = 0.10, init_range = 1.0) {
    self$n_centers <- n_centers
    self$log_range <- nn_parameter(torch_log(torch_tensor(init_range)))
    self$head <- nn_sequential(
      nn_linear(n_centers + 2, out_dim),
      nn_gelu(),
      nn_dropout(dropout)
    )
  },
  forward = function(coords, centers) {
    dx <- coords[, 1]$unsqueeze(2) - centers[, , 1]
    dy <- coords[, 2]$unsqueeze(2) - centers[, , 2]
    dist2 <- dx^2 + dy^2
...
GeoSplineNet <- nn_module(
  ...
  initialize = function(c_tab,
    ...
    self$spline_encoder <- RadialSplineEncoder(n_centers = n_centers, out_dim = d, dropout = dropout)
    self$fuse <- nn_sequential(
      nn_linear(2 * d, d),
      nn_gelu(),
      nn_dropout(dropout)
    )
    self$target_head <- GeoSplineScalarHead(d = d, hidden = 96, dropout = dropout)
    self$neighbor_head <- GeoSplineScalarHead(d = d, hidden = 96, dropout = dropout)
  ...
  encode_points = function(x, coords, centers) {
    z_tab <- self$tab_encoder(x)
    z_spline <- self$spline_encoder(coords, centers)
    self$fuse(torch_cat(list(z_tab, z_spline), dim = 2))
  },

  encode_neighbors = function(x_neighbors, coords_neighbors, centers) {
    ...
    centers_flat <- reshape_safe(centers, c(dims[1] * dims[2], -1, 2))
    z <- self$encode_points(x_flat, c_flat, centers_flat)
    reshape_safe(z, c(dims[1], dims[2], self$d))
  },

  forward = function(x_target, coords_target, x_neighbors, coords_neighbors, y_neighbors, centers_target, centers_neighbors, use_residual = TRUE) {
    z_target <- self$encode_points(x_target, coords_target, centers_target)
    z_neighbors <- self$encode_neighbors(x_neighbors, coords_neighbors, centers_neighbors)
...
predict_geosplinenet <- function(model,
                                 X_query,
                                 coords_query,
                                 neighbor_idx,
                                 X_ref,
  ...
  with_no_grad({
    for (s in seq(1, n, by = batch_size)) {
      ...
      centers_target <- to_float_tensor(expand_centers_array(centers, B), device = device)
      centers_neighbors <- centers_target$unsqueeze(2)$expand(c(B, K, nrow(centers), 2))

      pred <- as.numeric(
        model(xb, cb, xnb, cnb, ynb, centers_target, centers_neighbors)$pred$to(device = "cpu")
      )

      if (length(pred) != length(idx)) {
        stop(sprintf(
          "GeoSplineNet prediction length mismatch: expected %d values, got %d.",
...
train_geosplinenet_one_fold <- function(fd,
                                        epochs = 40,
                                        lr = 2e-4,
                                        wd = 1e-3,
                                        batch_size = 96,
  ...
  if (warmup_epochs > 0) {
    warmup_params <- c(
      ...
      model$spline_encoder$parameters,
    ...
    for (ep in seq_len(warmup_epochs)) {
      ...
      for (batch_id in seq_along(batches)) {
        ...
        out <- model(xb, cb, xnb, cnb, ynb, centers_target, centers_neighbors, use_residual = FALSE)
      ...
      cat(sprintf("[GeoSplineNet Warmup] Epoch %d/%d | train_loss=%.4f\n",
  ...
  for (ep in seq_len(epochs)) {
    ...
    for (batch_id in seq_along(batches)) {
      ...
      out <- model(xb, cb, xnb, cnb, ynb, centers_target, centers_neighbors, use_residual = TRUE)
      ...
      if (batch_id %% 10 == 0 || batch_id == length(batches)) {
        cat(sprintf("[GeoSplineNet] Epoch %d | batch %d/%d | batch_loss=%.4f\n",
                    ep, batch_id, length(batches), loss$item()))
      }
    }

    val_pred_s <- predict_geosplinenet(
      model = model,
      X_query = Xva,
      coords_query = Cva,
    ...
    cat(sprintf("[GeoSplineNet] Epoch %d complete | train_loss=%.4f | val_mse=%.4f\n",
  ...
  preds_scaled <- predict_geosplinenet(
    model = model,
    X_query = Xte,
    coords_query = Cte,
...
geosplinenet_quick_params <- modifyList(
  geosplinenet_params,
  list(
    epochs = 20,
    batch_size = 64,
...
run_geosplinenet_on_fixed_benchmark <- function(benchmark,
                                                context = wadoux_context,
                                                model_params = geosplinenet_params,
                                                K_neighbors = 12,
                                                n_centers = 32) {
  ...
  for (i in seq_along(benchmark$splits)) {
    ...
    cat(sprintf("\n[GeoSplineNet Fair] split %s | K_neighbors=%d | centers=%d\n",
    ...
    out <- do.call(train_geosplinenet_one_fold, c(list(fd = fd), model_params))
...
run_geosplinenet_vs_cubist_fair <- function(context = wadoux_context,
                                            sample_size = 250,
                                            ...
                                            max_splits = 5,
                                            seed = 123,
                                            model_params = geosplinenet_quick_params,
                                            K_neighbors = 12,
                                            n_centers = 32,
                                            cubist_committees = 50,
                                            cubist_neighbors = 5,
                                            results_dir = "results/geosplinenet_vs_cubist",
                                            save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoSplineNet vs Cubist BENCHMARK\n")
  cat("========================================\n")
  ...
  geo_res <- run_geosplinenet_on_fixed_benchmark(
    benchmark = benchmark,
    context = context,
  ...
  if (save_outputs) {
    ...
    write.csv(geo_res, file.path(results_dir, "geosplinenet_results.csv"), row.names = FALSE)
    write.csv(final, file.path(results_dir, "geosplinenet_vs_cubist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geosplinenet_vs_cubist_summary.csv"), row.names = FALSE)
  }

  final
}

run_geosplinenet_vs_cubist_confirmation <- function(context = wadoux_context,
                                                    sample_size = 300,
                                                    sampling = "simple_random",
                                                    n_folds = 10,
                                                    val_dist_km = 350,
                                                    ...
                                                    cubist_committees = 50,
                                                    cubist_neighbors = 5,
                                                    results_dir = "results/geosplinenet_vs_cubist_confirmation",
                                                    save_outputs = TRUE) {
  run_geosplinenet_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
...
make_geosplinenet_shortlist_variants <- function() {
  list(
    GeoSplineNet_C16_K8 = list(n_centers = 16, K_neighbors = 8, params = geosplinenet_quick_params),
    GeoSplineNet_C16_K12 = list(n_centers = 16, K_neighbors = 12, params = geosplinenet_quick_params),
    GeoSplineNet_C32_K8 = list(n_centers = 32, K_neighbors = 8, params = geosplinenet_quick_params),
    GeoSplineNet_C32_K12 = list(n_centers = 32, K_neighbors = 12, params = geosplinenet_quick_params),
    GeoSplineNet_C48_K8 = list(n_centers = 48, K_neighbors = 8, params = geosplinenet_quick_params),
    GeoSplineNet_C48_K12 = list(n_centers = 48, K_neighbors = 12, params = geosplinenet_quick_params)
  )
}

run_geosplinenet_shortlist_search <- function(context = wadoux_context,
                                              sample_size = 250,
                                              sampling = "simple_random",
                                              n_folds = 5,
                                              val_dist_km = 350,
                                              ...
                                              max_splits = 5,
                                              seed = 123,
                                              variants = make_geosplinenet_shortlist_variants(),
                                              cubist_committees = 50,
                                              cubist_neighbors = 5,
                                              results_dir = "results/geosplinenet_shortlist_search",
                                              save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GEOSPLINENET SHORTLIST BENCHMARK\n")
  cat("========================================\n")
  ...
  for (variant_name in names(variants)) {
    ...
    cat(sprintf("\n[GeoSpline Shortlist] %s | centers=%d | K=%d\n",
                variant_name, var_cfg$n_centers, var_cfg$K_neighbors))

    geo_res <- run_geosplinenet_on_fixed_benchmark(
      benchmark = benchmark,
      context = context,
      model_params = var_cfg$params,
      K_neighbors = var_cfg$K_neighbors,
      n_centers = var_cfg$n_centers
    ) %>%
      mutate(variant = variant_name)

    cubist_variant <- cubist_res %>% mutate(variant = variant_name)
    all_results[[variant_name]] <- bind_rows(cubist_variant, geo_res)
  ...
  if (save_outputs) {
    write.csv(final, file.path(results_dir, "geosplinenet_shortlist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geosplinenet_shortlist_summary.csv"), row.names = FALSE)
  }

  final
}

run_geosplinenet_c32k8_confirmation <- function(context = wadoux_context,
                                                sample_size = 300,
                                                sampling = "simple_random",
                                                n_folds = 10,
                                                val_dist_km = 350,
                                                ...
                                                cubist_committees = 50,
                                                cubist_neighbors = 5,
                                                results_dir = "results/geosplinenet_c32k8_confirmation",
                                                save_outputs = TRUE) {
  run_geosplinenet_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
...
# Example:
# source("code/GeoSplineNet.R")
# res_geospline <- run_geosplinenet_vs_cubist_fair(
#   context = wadoux_context,
#   sample_size = 250,
#   n_folds = 5,
#   max_splits = 5,
