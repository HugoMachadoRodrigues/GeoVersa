...
# -----------------------------------------------------------------------------
# Point-based pedometric transformer with:
# - feature-token transformer for the target point
# - shared point encoder for target and neighbors
# - light anisotropic residual kriging-like correction
#
# Plain-language flow:
# 1. Turn each predictor into a token.
# 2. Add a coordinate token and a CLS token.
# 3. Use self-attention to learn cross-feature structure at each point.
# 4. Predict a base value for the target and for neighbors.
...
prepare_pedoformer_fold <- function(context,
                                    calibration_df,
                                    train_idx,
                                    val_idx,
                                    test_idx,
...
FeatureMultiHeadAttention <- nn_module(
  "FeatureMultiHeadAttention",
  initialize = function(d_model = 128, n_heads = 4, dropout = 0.10) {
...
FeatureTransformerBlock <- nn_module(
  "FeatureTransformerBlock",
  initialize = function(d_model = 128, n_heads = 4, ff_mult = 2, dropout = 0.10) {
    self$norm1 <- nn_layer_norm(normalized_shape = d_model)
    self$norm2 <- nn_layer_norm(normalized_shape = d_model)
    self$attn <- FeatureMultiHeadAttention(d_model = d_model, n_heads = n_heads, dropout = dropout)
...
PedoFeatureTokenizer <- nn_module(
  "PedoFeatureTokenizer",
  initialize = function(c_tab, d = 128, dropout = 0.10) {
    self$c_tab <- c_tab
    self$d <- d
    self$feature_weight <- nn_parameter(torch_randn(c(c_tab, d)) * 0.02)
    self$feature_bias <- nn_parameter(torch_zeros(c(c_tab, d)))
    self$cls_token <- nn_parameter(torch_zeros(c(1, 1, d)))
    self$geo_token <- nn_sequential(
      nn_linear(2, d),
      nn_gelu(),
      nn_linear(d, d)
    )
  ...
  forward = function(x, coords) {
    dims <- as.integer(x$shape)
    B <- dims[1]
    feat_tokens <- x$unsqueeze(3) * self$feature_weight$unsqueeze(1) + self$feature_bias$unsqueeze(1)
    geo_token <- self$geo_token(coords)$unsqueeze(2)
    cls <- self$cls_token$expand(c(B, 1, self$d))
    self$drop(torch_cat(list(cls, feat_tokens, geo_token), dim = 2))
  }
)

PedoformerPointEncoder <- nn_module(
  "PedoformerPointEncoder",
  initialize = function(c_tab,
                        ...
                        dropout = 0.10) {
    self$d <- d
    self$tokenizer <- PedoFeatureTokenizer(c_tab = c_tab, d = d, dropout = dropout)
    blocks <- vector("list", n_layers)
    for (i in seq_len(n_layers)) {
      blocks[[i]] <- FeatureTransformerBlock(
        d_model = d,
        n_heads = n_heads,
        ff_mult = ff_mult,
        dropout = dropout
    ...
    self$norm <- nn_layer_norm(normalized_shape = d)
  },
  forward = function(x, coords) {
    h <- self$tokenizer(x, coords)
    ...
    h <- self$norm(h)
    h$narrow(2, 1, 1)$squeeze(2)
  }
)

PedoformerScalarHead <- nn_module(
  "PedoformerScalarHead",
...
PedoformerKrigingNet <- nn_module(
  "PedoformerKrigingNet",
  initialize = function(c_tab,
                        d = 128,
                        n_heads = 4,
                        ...
                        beta_init = -4) {
    self$d <- d
    self$point_encoder <- PedoformerPointEncoder(
      c_tab = c_tab,
      d = d,
      n_heads = n_heads,
      n_layers = n_layers,
    ...
    self$target_head <- PedoformerScalarHead(d = d, hidden = 96, dropout = dropout)
    self$neighbor_head <- PedoformerScalarHead(d = d, hidden = 96, dropout = dropout)
  ...
  encode_neighbors = function(x_neighbors, coords_neighbors) {
    ...
    z <- self$point_encoder(x_flat, c_flat)
  ...
  forward = function(x_target, coords_target, x_neighbors, coords_neighbors, y_neighbors, use_residual = TRUE) {
    z_target <- self$point_encoder(x_target, coords_target)
...
predict_pedoformer <- function(model,
                               X_query,
                               coords_query,
                               neighbor_idx,
                               X_ref,
  ...
  with_no_grad({
    for (s in seq(1, n, by = batch_size)) {
      ...
      if (length(pred) != length(idx)) {
        stop(sprintf(
          "Pedoformer prediction length mismatch: expected %d values, got %d.",
...
train_pedoformer_one_fold <- function(fd,
                                      epochs = 40,
                                      lr = 2e-4,
                                      wd = 1e-3,
                                      batch_size = 96,
  ...
  if (warmup_epochs > 0) {
    warmup_params <- c(
      model$point_encoder$parameters,
    ...
    for (ep in seq_len(warmup_epochs)) {
      ...
      cat(sprintf("[Pedoformer Warmup] Epoch %d/%d | train_loss=%.4f\n",
  ...
  for (ep in seq_len(epochs)) {
    ...
    for (batch_id in seq_along(batches)) {
      ...
      if (batch_id %% 10 == 0 || batch_id == length(batches)) {
        cat(sprintf("[Pedoformer] Epoch %d | batch %d/%d | batch_loss=%.4f\n",
                    ep, batch_id, length(batches), loss$item()))
      }
    }

    val_pred_s <- predict_pedoformer(
      model = model,
      X_query = Xva,
      coords_query = Cva,
    ...
    cat(sprintf("[Pedoformer] Epoch %d complete | train_loss=%.4f | val_mse=%.4f\n",
  ...
  preds_scaled <- predict_pedoformer(
    model = model,
    X_query = Xte,
    coords_query = Cte,
...
pedoformerkrigingnet_params <- list(
  epochs = 40,
  lr = 2e-4,
  wd = 1e-3,
  batch_size = 96,
...
pedoformerkrigingnet_quick_params <- modifyList(
  pedoformerkrigingnet_params,
  list(
    epochs = 20,
    batch_size = 64,
    patience = 5,
...
run_pedoformer_on_fixed_benchmark <- function(benchmark,
                                              context = wadoux_context,
                                              model_params = pedoformerkrigingnet_params,
                                              K_neighbors = 12) {
  results <- vector("list", length(benchmark$splits))

  for (i in seq_along(benchmark$splits)) {
    sp <- benchmark$splits[[i]]
    cat(sprintf("\n[Pedoformer Fair] split %s | K_neighbors=%d\n",
    ...
    out <- do.call(train_pedoformer_one_fold, c(list(fd = fd), model_params))
...
run_pedoformerkrigingnet_vs_cubist_fair <- function(context = wadoux_context,
                                                     sample_size = 250,
                                                     ...
                                                     max_splits = 5,
                                                     seed = 123,
                                                     model_params = pedoformerkrigingnet_quick_params,
                                                     K_neighbors = 12,
                                                     cubist_committees = 50,
                                                     cubist_neighbors = 5,
                                                     results_dir = "results/pedoformerkrigingnet_vs_cubist",
                                                     save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING PedoformerKrigingNet vs Cubist BENCHMARK\n")
  cat("========================================\n")
  ...
  pedo_res <- run_pedoformer_on_fixed_benchmark(
    benchmark = benchmark,
    context = context,
  ...
  final <- bind_rows(cubist_res, pedo_res)
  summary_tbl <- final %>%
    group_by(model) %>%
    summarise(
  ...
  if (save_outputs) {
    ...
    write.csv(pedo_res, file.path(results_dir, "pedoformerkrigingnet_results.csv"), row.names = FALSE)
    write.csv(final, file.path(results_dir, "pedoformerkrigingnet_vs_cubist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "pedoformerkrigingnet_vs_cubist_summary.csv"), row.names = FALSE)
  }

  final
}

run_pedoformerkrigingnet_vs_cubist_confirmation <- function(context = wadoux_context,
                                                             sample_size = 300,
                                                             sampling = "simple_random",
                                                             n_folds = 10,
                                                             val_dist_km = 350,
                                                             ...
                                                             cubist_committees = 50,
                                                             cubist_neighbors = 5,
                                                             results_dir = "results/pedoformerkrigingnet_vs_cubist_confirmation",
                                                             save_outputs = TRUE) {
  run_pedoformerkrigingnet_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
