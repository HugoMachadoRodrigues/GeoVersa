...
# - neural additive model over tabular covariates
# - learned spatial basis encoder from coordinates
# - fused global base prediction
# - light anisotropic residual kriging-like correction
#
# Plain-language flow:
# 1. Each covariate gets its own small neural function.
# 2. These feature-wise effects are summed into a stable additive representation.
# 3. A spatial basis encoder adds smooth geographic structure.
# 4. Both parts form a global base prediction.
# 5. Neighbor residuals provide a light anisotropic spatial correction.
# =============================================================================
...
sample_nam_centers <- function(coords_train, n_centers = 32, seed = 123) {
  set.seed(seed)
...
prepare_geonam_fold <- function(context,
                                calibration_df,
                                train_idx,
                                val_idx,
                                test_idx,
  ...
  init_centers <- sample_nam_centers(coords_train_s, n_centers = n_centers, seed = center_seed)
...
NAMFeatureBranch <- nn_module(
  "NAMFeatureBranch",
  initialize = function(hidden = 24, out_dim = 160, dropout = 0.10) {
  ...
  forward = function(x_j) {
    ...
    feat <- torch_cat(list(x_col, x_col^2), dim = 2)
    self$net(feat)
  }
)

GeoNAMAdditiveEncoder <- nn_module(
  "GeoNAMAdditiveEncoder",
  initialize = function(c_tab,
    ...
    branches <- lapply(seq_len(self$n_features), function(i) {
      NAMFeatureBranch(hidden = branch_hidden, out_dim = d, dropout = dropout)
    })
  ...
  forward = function(x) {
    ...
    acc <- self$effect_scale * acc / sqrt(self$n_features)
    acc + self$post_scale * self$post(acc)
  }
)

GeoNAMScalarHead <- nn_module(
  "GeoNAMScalarHead",
...
GeoNAMKrigingNet <- nn_module(
  ...
  initialize = function(c_tab,
                        ...
                        dropout = 0.10,
                        beta_init = -4) {
    self$nam_scale <- nam_scale
    self$basis_scale <- basis_scale
    self$fuse_scale <- fuse_scale
    self$d <- d
    self$nam_encoder <- GeoNAMAdditiveEncoder(
      c_tab = c_tab,
      d = d,
      branch_hidden = branch_hidden,
      post_hidden = post_hidden,
      effect_scale = nam_effect_scale,
      post_scale = nam_post_scale,
      dropout = dropout
    )
    ...
    self$target_head <- GeoNAMScalarHead(d = d, hidden = 96, dropout = dropout)
    self$neighbor_head <- GeoNAMScalarHead(d = d, hidden = 96, dropout = dropout)
  ...
  encode_points = function(x, coords) {
    z_nam <- self$nam_encoder(x)
    z_basis <- self$basis_encoder(coords)
    self$nam_scale * z_nam +
      self$basis_scale * z_basis +
      self$fuse_scale * self$fuse_residual(torch_cat(list(z_nam, z_basis), dim = 2))
  },
  ...
  with_no_grad({
    for (s in seq(1, n, by = batch_size)) {
      ...
      if (length(pred) != length(idx)) {
        stop(sprintf(
          "GeoNAMKrigingNet prediction length mismatch: expected %d values, got %d.",
...
train_geonam_one_fold <- function(fd,
                                  epochs = 40,
                                  lr = 2e-4,
                                  wd = 1e-3,
                                  batch_size = 96,
  ...
  model <- GeoNAMKrigingNet(
    c_tab = ncol(Xtr),
    init_centers = fd$init_centers,
    d = d,
    branch_hidden = branch_hidden,
    post_hidden = post_hidden,
    nam_effect_scale = nam_effect_scale,
    nam_post_scale = nam_post_scale,
  ...
  if (warmup_epochs > 0) {
    ...
    for (ep in seq_len(warmup_epochs)) {
      ...
      cat(sprintf("[GeoNAM Warmup] Epoch %d/%d | train_loss=%.4f\n",
  ...
  for (ep in seq_len(epochs)) {
    ...
    for (batch_id in seq_along(batches)) {
      ...
      if (batch_id %% 10 == 0 || batch_id == length(batches)) {
        cat(sprintf("[GeoNAM] Epoch %d | batch %d/%d | batch_loss=%.4f\n",
                    ep, batch_id, length(batches), loss$item()))
      }
    }

    val_pred_s <- predict_geonam(
      model = model,
      X_query = Xva,
      coords_query = Cva,
    ...
    cat(sprintf("[GeoNAM] Epoch %d complete | train_loss=%.4f | val_mse=%.4f\n",
...
geonamkrigingnet_quick_params <- modifyList(
  geonamkrigingnet_params,
  list(
    epochs = 20,
    batch_size = 64,
    patience = 5,
...
run_geonam_on_fixed_benchmark <- function(benchmark,
                                          context = wadoux_context,
                                          model_params = geonamkrigingnet_params,
                                          K_neighbors = 12,
                                          n_centers = 32) {
  ...
  for (i in seq_along(benchmark$splits)) {
    ...
    cat(sprintf("\n[GeoNAM Fair] split %s | K_neighbors=%d | centers=%d\n",
    ...
    out <- do.call(train_geonam_one_fold, c(list(fd = fd), model_params))
...
run_geonamkrigingnet_vs_cubist_fair <- function(context = wadoux_context,
                                                sample_size = 250,
                                                ...
                                                max_splits = 5,
                                                seed = 123,
                                                model_params = geonamkrigingnet_quick_params,
                                                K_neighbors = 12,
                                                n_centers = 32,
                                                cubist_committees = 50,
                                                cubist_neighbors = 5,
                                                results_dir = "results/geonamkrigingnet_vs_cubist",
                                                save_outputs = TRUE) {
  dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

  cat("\n========================================\n")
  cat("BUILDING GeoNAMKrigingNet vs Cubist BENCHMARK\n")
  cat("========================================\n")
  ...
  geo_res <- run_geonam_on_fixed_benchmark(
    benchmark = benchmark,
    context = context,
  ...
  if (save_outputs) {
    ...
    write.csv(geo_res, file.path(results_dir, "geonamkrigingnet_results.csv"), row.names = FALSE)
    write.csv(final, file.path(results_dir, "geonamkrigingnet_vs_cubist_all.csv"), row.names = FALSE)
    write.csv(summary_tbl, file.path(results_dir, "geonamkrigingnet_vs_cubist_summary.csv"), row.names = FALSE)
  }

  final
}

run_geonamkrigingnet_vs_cubist_confirmation <- function(context = wadoux_context,
                                                        sample_size = 300,
                                                        sampling = "simple_random",
                                                        n_folds = 10,
                                                        val_dist_km = 350,
                                                        ...
                                                        cubist_committees = 50,
                                                        cubist_neighbors = 5,
                                                        results_dir = "results/geonamkrigingnet_vs_cubist_confirmation",
                                                        save_outputs = TRUE) {
  run_geonamkrigingnet_vs_cubist_fair(
    context = context,
    sample_size = sample_size,
    sampling = sampling,
    n_folds = n_folds,
